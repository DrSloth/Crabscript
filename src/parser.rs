...
impl Parser {
            ...
            KeywordToken::If => {
                ...
                loop {
                    match self.next_token(&mut tokens)? {
                        ...
                        next => {
                            let (b, ts) = self.parse_branch_inner(Some(next), tokens)?;
                            tokens = ts;
                            if let Some(b) = b {
                                branches.push(b)
                            } else {
    ...
    fn decl_inner<'node, 'text, 'tokens>(
        ...
        let id = expect!(self.next_token(&mut tokens)? => Token::Identifier | "Expected identifier");
