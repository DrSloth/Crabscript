use crate::{
    base::{Args, DayObject, RustFunction},
    node::NodePurpose,
    parser::parsing_error::ParsingResult,
    parser::Parser,
    std_modules,
    tokenizer::{build_lexer, TokenStream},
...
pub fn run(src: &str) -> ParsingResult<()> {
    ...
    let root_node = parser
        .parse(TokenStream::new(tokens), NodePurpose::TopLevel)?
        .0;
