...
impl<'b, 'ret, 'a: 'ret> Variables<'a> {
    ...
    pub fn new() -> Self {
        Self {
            vars: UnsafeCell::new(HashMap::with_hasher(super::get_global_hasherbuilder())),
            ..Default::default()
        }
    }
    ...
    pub fn def_fn<'key>(self: &Arc<Self>, hash: u64, key: &'a str, value: Arc<RootNode<'a>>) -> usize {
        unsafe {
            match (*self.vars.get()).get(&key) {
                None => {
                    let len = (*self.funcs.get()).len();
                    self.clone()
                        .def_const_ref_hash(hash, key, DayObject::Function(DayFunction::RuntimeDef(len)));
